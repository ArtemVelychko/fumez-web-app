...
const pyramidLevels = [
  ...
  { value: "baseHeart", src: "/pyramid-base-mid.svg", tooltip: "Base/Heart Note" },
...
export const DilutionsPopover: React.FC<DilutionsPopoverProps> = ({ dilutions, onChange }) => {
          ...
          {dilutions.map((dilution, index) => (
            <div key={index} className="flex items-center">
              <Input
                ...
                max={100}
                value={dilution}
                onChange={(e) => handleDilutionChange(index, Number(e.target.value))}
              />
              <span className="mx-2">%</span>
              <Button variant="ghost" size="sm" onClick={() => handleRemoveDilution(index)}>
...
export const DatePickerPopover: React.FC<DatePickerPopoverProps> = ({ date, onSelect }) => (
  <Popover>
    <PopoverTrigger asChild>
      <Button
        variant="outline"
